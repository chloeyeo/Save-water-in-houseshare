      ...
      <div className="bg-white rounded p-4 z-30 w-64 h-84" >
        ...
        <div class="flex justify-center">
          <div class="timepicker relative form-floating mb-3 xl:w-96">
            ...
            <input type="text"
              class="form-control block w-full px-3 py-1.5 text-sm font-medium text-gray-900 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
              placeholder="Select a date" />
          </div>
        </div>
        <input class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="checkbox" value="" id="flexCheckDefault" />
        <label class="mb-2 form-check-label inline-block text-sm text-gray-900" for="flexCheckDefault">
          Recurring activity
        </label>
        <button onClick={() => addTask()} className="z-31 p-6 block add-button modal-footer justify-end">
          Add
        </button>
